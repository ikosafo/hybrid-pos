...
class ProductModel {
    ...
    public static function update(int $id, array $d): bool {
        ...
        $price         = (float)$d['price'];
        $costPrice     = (float)$d['cost_price'];
        $stockQty      = (float)$d['stock_qty'];
        $lowStockAlert = (int)$d['low_stock_alert'];
        $unit          = (string)$d['unit'];
        $trackStock    = (int)$d['track_stock'];
        $productId     = (int)$id;

        // i=int, s=string, d=decimal
        // category_id(i), name(s), sku(s), barcode(s), description(s),
        // price(d), cost_price(d), stock_qty(d), low_stock_alert(i),
        // unit(s), track_stock(i), id(i)
        $stmt->bind_param(
            'issssdddisii',
            $categoryId, $name, $sku, $barcode, $description,
            $price, $costPrice, $stockQty, $lowStockAlert,
            $unit, $trackStock, $productId
        );
